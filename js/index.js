...
class Quiz {
  ...
  async highlightingAnswers () {
    ...
    const answerButtons = this.#answerButtons()
  ...
  updateProgressBar () {
    ...
    const currentStep = this.questionNumber + 1
  ...
  createProgressBar () {
    const container = document.querySelector('.containerProgressBar')
  ...
  madeWarning (time, number) {
    ...
    if (this.questions[this.randomQuestions[this.questionNumber]].responsesNumber) {
      warning += `Select ${number || this.checkNumberOfAnswers()} answers\n`
    }
    return warning
  }
  ...
  updateQuestion () {
    ...
    const answers = this.randomizeArray(this.questions[this.randomQuestions[this.questionNumber]].answers)
